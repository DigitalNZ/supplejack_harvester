...
RSpec.describe JobCompletionSummary do
  describe 'associations' do
    it { is_expected.to have_many(:job_errors).dependent(:destroy) }
  end

  describe 'validations' do
    it 'validates presence of job_id' do
      summary = described_class.new(job_id: nil, job_type: 'ExtractionJob')
    ...
    it 'validates presence of job_type' do
      ...
      summary = described_class.new(job_id: job.id, job_type: nil)
    ...
    it 'enforces uniqueness of job_id scoped to process_type and job_type at database level' do
      ...
      duplicate = described_class.new(job_id: job.id, process_type: 'extraction', job_type: 'ExtractionJob')
  ...
  describe 'enums' do
    it 'defines process_type enum' do
      expect(described_class.process_types).to eq({ 'extraction' => 0, 'transformation' => 1 })
    end
    ...
    it 'can be created with transformation process_type' do
      ...
      summary = create(:job_completion_summary, job_id: harvest_job.id, process_type: :transformation,
  ...
  describe '#pipeline_name' do
    ...
    it 'returns nil when job not found' do
      job = create(:extraction_job)
      summary = create(:job_completion_summary, job_id: job.id)
      summary.update!(job_id: 999_999)

      expect(described_class.find_for_harvest_job(job.id)).to be_nil
    end
  end

  describe '.error_count_for_harvest_job' do
    it 'returns 0 when harvest_job_id is blank' do
      expect(described_class.error_count_for_harvest_job(nil)).to eq(0)
      expect(described_class.error_count_for_harvest_job('')).to eq(0)
    end

    it 'returns 0 when harvest_job not found' do
      expect(described_class.error_count_for_harvest_job(999_999)).to eq(0)
    end
    ...
    it 'returns error count for extraction summaries' do
      ...
      expect(described_class.error_count_for_harvest_job(harvest_job.id)).to eq(2)
    end
  end

  describe '.find_for_harvest_job' do
    it 'returns nil when harvest_job_id is blank' do
      expect(described_class.find_for_harvest_job(nil)).to be_nil
      expect(described_class.find_for_harvest_job('')).to be_nil
    end

    it 'returns nil when harvest_job not found' do
      expect(described_class.find_for_harvest_job(999_999)).to be_nil
    end
    ...
    it 'returns extraction summary when only extraction summary exists' do
      ...
      result = described_class.find_for_harvest_job(harvest_job.id)

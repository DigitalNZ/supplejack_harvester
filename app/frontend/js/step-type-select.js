document.addEventListener('DOMContentLoaded', function() {
  const stepTypeSelect = document.querySelector('select[name="automation_step_template[step_type]"]');
  
  if (stepTypeSelect) {
    const pipelineFields = document.getElementById('pipeline-fields');
    const apiCallFields = document.getElementById('api-call-fields');
    const pipelineSelect = document.querySelector('select[name="automation_step_template[pipeline_id]"]');

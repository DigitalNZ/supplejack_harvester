...
const LoadSchema = () => {
  ...
  const schemaFields = () => {
    const activeSchema = find(allSchemas, (schema) => {
      return schema.id == schemaValue;
    });
  ...
  const updateSchemaFieldIds = (id) => {
    if (includes(schemaFieldIds, id)) {
      ...
      setSchemaFieldIds(filteredValues);
    } else {
      setSchemaFieldIds([...schemaFieldIds, id]);
    }
  };

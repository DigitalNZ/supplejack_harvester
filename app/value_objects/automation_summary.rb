...
class AutomationSummary
  ...
  def calculate_metrics_for_step(step, previous_end_time)
    ...
    timing_metrics[:duration] = (step_end_time - step_start_time).to_i if step_start_time && step_end_time

    timing_metrics[:waiting_time] = (step_start_time - previous_end_time).to_i if previous_end_time && step_start_time
    ...
    if step.pipeline_job&.harvest_reports.present?
      ...
      if timing_metrics[:duration].positive? && harvest_metrics[:active_time].positive?

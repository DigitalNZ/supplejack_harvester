...
module JobsHelper
  ...
  def job_transformation_input(job)
    ...
    link_to "##{extraction_job.id}", pipeline_harvest_definition_extraction_definition_extraction_job_path(
      job.pipeline.id,
  ...
  def job_launched_by_label(pipeline_job)
    ...
    elsif (automation_step = automation_step.presence)
